...
(defun  altera-dominio (psr inferencias)
  (let ((vars (psr-variaveis-todas psr))
    ...
    (dolist (dominio (psr-dominio psr))
      (setf var (first vars))
      (when (dolist (inferencia inferencias)
        (when (equal (inferencia-variavel inferencia) var)
          (setf dominios-original (append dominios-original (cria-inferencia var dominio)))
          (setf dominios-novo (append dominios-novo (list (inferencia-dominio inferencia))))
          (return t)
        )
      ) (setf dominios-novo (append dominios-novo dominio)))
    )
  ...
  (let ((result-num 0)
    ...
    (dolist (valor-dominio-v1 dominio-v1)
      (when (not (dolist (valor-dominio-v2 dominio-v2)
        ...
        (print (write-to-string valor-dominio-v1))
        (print (write-to-string novo-dominio-v1))
      )
    )
    (when revised (setf inferencias (adiciona-inferencia v1 novo-dominio-v1 inferencias)))
    (values revised result-num)
  )
)
...
(defun forward-checking (psr var)
  (let ((result-num 0)
    ...
    (print (write-to-string lista-arcos))
    (cond ((dolist (arco lista-arcos t)
      (multiple-value-bind (result-p result-num-aux) (revise psr (car arco) (cdr arco) inferencias)
        (print (write-to-string arco))
        (print (write-to-string inferencias))
        (print (write-to-string result-p))
        (print (write-to-string result-num-aux))
        (incf result-num result-num-aux)
        (when result-p
          ...
          (multiple-value-bind (result-p result-num-aux) (psr-atribuicao-consistente-p psr variavel valor)
            ...
            (cond (result-p
                ...
                (multiple-value-bind (result-p-fd result-num-fd) (forward-checking psr variavel)
                  (print (write-to-string result-p-fd))
                  (print (write-to-string result-num-fd))
                  (incf result-num result-num-fd)
                  (cond (result-p-fd
                    (setf dominios-aux (altera-dominio psr result-p-fd))
                    (print (write-to-string dominios-aux))
