...
;verifica se a variavel nao pertence a lista
...
;verifica se a variavel esta na lista de variaveis da restricao
...
;aplica a lista de restricoes ao psr
    ...
    (dolist (var-natribuida (psr-variaveis-nao-atribuidas psr) lista-arcos)
      (when (not (equal var var-natribuida))
        (when (envolvida-restricao psr var var-natribuida)
          (setf lista-arcos (append lista-arcos (list (list var-natribuida var))))
        )
      )
    )
  )
...
(defun  altera-dominio (psr inferencias)
  (let ((dominios-original nil))
    (dolist (inferencia inferencias)
      ...
      (psr-altera-dominio! psr (inferencia-variavel inferencia) (inferencia-dominio inferencia))
    )
...
(defun adiciona-inferencia (variavel dominio lista)
  (cond ((null lista) (list (cria-inferencia variavel dominio)))
    ...
    (t (append (list (first lista)) (adiciona-inferencia variavel dominio (rest lista))))
  )  
)
...
(defun revise (psr v1 v2 inferencias)
  (let ((result-num 0)
    ...
    (dolist (valor-dominio-v1 dominio-v1)
      (when (not (dolist (valor-dominio-v2 dominio-v2)
        ...
        (setf revised t)
        (setf novo-dominio-v1 (remove-lista valor-dominio-v1 novo-dominio-v1))
      )
    )
    (when revised (setf inferencias (adiciona-inferencia v1 novo-dominio-v1 inferencias)))
    (values revised result-num inferencias)
  )
)
...
(defun funcao-inferencias (psr var funcao-altera-arcos)
  (let ((result-num 0)
    ...
    (cond ((do* ((lista-arcos (arcos-vizinhos-nao-atribuidos psr var) (rest lista-arcos))
                 (arco (first lista-arcos) (first lista-arcos)))
                ((null lista-arcos) t)        
      (multiple-value-bind (result-p result-num-aux inferencias-aux) (revise psr (first arco) (second arco) inferencias)
        (setf inferencias inferencias-aux)
        (incf result-num result-num-aux)
        (when result-p
          (when (= (list-length (inferencia-variavel-dominio inferencias (first arco))) 0)
            (return nil)
          )
          (when funcao-altera-arcos
            (setf lista-arcos (funcall funcao-altera-arcos arco (arcos-vizinhos-nao-atribuidos psr (first arco)) lista-arcos))
          )
        )
      )
    ) (values t inferencias result-num))
    (t (values nil nil result-num)))
  )
)
...
(defun MAC (psr var)
  (funcao-inferencias psr var #'altera-arcos)
)
        ...
        (dolist (valor (psr-variavel-dominio psr variavel) (values nil result-num))
          (multiple-value-bind (result-p result-num-aux) (psr-atribuicao-consistente-p psr variavel valor)
            ...
            (cond (result-p
                ...
                (multiple-value-bind (result-p-fd result-inferencias result-num-fd) (funcall funcao-inferencias psr variavel)
