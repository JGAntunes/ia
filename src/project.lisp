...
(defun  altera-dominio (psr inferencias)
  (let ((vars (psr-variaveis-todas psr))
    ...
    (dolist (dominio (psr-dominio psr))
      (setf var (first vars))
      (when (not (dolist (inferencia inferencias)
        (when (equal (inferencia-variavel inferencia) var)
          (setf dominios-original (append dominios-original (list (cria-inferencia var dominio))))
          (setf dominios-novo (append dominios-novo (list (inferencia-dominio inferencia))))
          (return t)
        )
      )) (setf dominios-novo (append dominios-novo (list dominio))))
      (setf vars (rest vars))
    )
        ...
        (dolist (valor (psr-variavel-dominio psr variavel) (values nil result-num))
          (multiple-value-bind (result-p result-num-aux) (psr-atribuicao-consistente-p psr variavel valor)
            ...
            (cond (result-p
                ...
                (multiple-value-bind (result-p-fd result-inferencias result-num-fd) (forward-checking psr variavel)
                  (incf result-num result-num-fd)
                  (cond (result-p-fd
                    (setf dominios-aux (altera-dominio psr result-inferencias))
                    (multiple-value-bind (result-p-proc result-num-proc) (procura-retrocesso-fc-mrv psr)
...
(defun procura-retrocesso-mac (psr)
  (let ((result-num 0)
    (variavel nil)
    (dominios-aux nil))
    (cond ((psr-completo-p psr) (values psr result-num))
